...
int main(int argc, char* argv[]){
    ...
    for (uint32_t x = 0; x < grid_x; x++)
        for (uint32_t y = 0; y < grid_y; y++){
            grid[(y * grid_x) + x].flip_on = rng_next8(&rng) < 20;
        }
        ...
        for (uint32_t x = 0; x < grid_x; x++)
            for (uint32_t y = 0; y < grid_y; y++){
                if (grid[(y * grid_x) + x].flip_on){
                    ...
                    grid[(y * grid_x) + x].flip_on = false;
                    grid[(y * grid_x) + x].alive = true;
                }
                if (grid[(y * grid_x) + x].flip_off){
                    fb_fill_rect(&ctx, x * SCALE, y * SCALE, SCALE, SCALE, BG_COLOR);
                    grid[(y * grid_x) + x].flip_off = false;
                    grid[(y * grid_x) + x].alive = false;
                }
            }
        commit_draw_ctx(&ctx);
        check_neighbors();
